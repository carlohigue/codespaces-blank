...
const AddMeeting = (props) => {
    ...
    const fetchAllData = async () => {
        ...
        const result1 = await dispatch(fetchContactData());
        if (result1?.payload != null) {
            setContactData(result1?.payload);
        }

        const result2 = await dispatch(fetchLeadData());
        if (result2?.payload != null) {
            setLeadData(result2?.payload);
        }

        if (result1?.payload == null || result2?.payload == null) {
    ...
    useEffect(() => {
        setContactModel(values.related === "Contact" ? true : false)
        setLeadModel(values.related === "Lead" ? true : false);
        fetchAllData();
